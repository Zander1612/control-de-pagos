...
const createNavTodos = () => {
    navbar.innerHTML = `
        <div class="max-width-7xl h-16 mx-auto flex items-center px-4 justify-between">
        <p class="font-bold text-sm text-white">Control de Pagos</p>
...
if(window.location.pathname === '/') {
    ...
    createNavLogin();
} else if (window.location.pathname === '/users/' || window.location.pathname === '/users/' || window.location.pathname === '/admin/') {
    createNavTodos();
}

// Lógica del menú móvil (se mantiene tu lógica de navegación por hijos)
const navBtn = navbar.children[0].children[1];

if (navBtn && navBtn.tagName === 'svg') {
...
// SECCIÓN DE LOGOUT AGREGADA (Solo se ejecuta si existen los botones en /todos, /users o /admin)
const closeBtnMobile = navbar.querySelector('.bg-slate-900/80 #close-btn');
const closeBtnDesktop = navbar.querySelector('.hidden.md\\:flex #close-btn');
...
if (closeBtnMobile) {
    closeBtnMobile.addEventListener('click', handleLogout);
}

if (closeBtnDesktop) {
    closeBtnDesktop.addEventListener('click', handleLogout);
}